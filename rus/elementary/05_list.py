...
list.extend() - расширить первый список элементами второго
str.split() - разбить строку на список подстрок
"""
...
# При умножении списка на число n получается новый список, составленный из n повторений исходного

one = [1, 'one']
print(one * 3) # [1, 'one', 1, 'one', 1, 'one']
...
# Вместо этого в строчке wtf = mylist с нашим списком связывается еще одно имя wtf, поэтому обращаясь к элементам wtf, мы меняем все тот же список
...
# Чтобы разбить строку на список из подстрок, используйте split()

mylist = ['what the foo']
print(mylist.split()) # ['what', 'the', 'foo']

# Если нужные подстроки разделены, например, запятыми, то можно указать этот разделитель первым параметром в split()
